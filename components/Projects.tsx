...
function ProjectCard({ project, index, isInView }: { project: (typeof projects)[0]; index: number; isInView: boolean }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.15 * index, duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] as const }}>
      <div ref={cardRef} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave} onMouseEnter={() => setIsHovered(true)} className="group perspective-[1200px] cursor-pointer">
        <motion.div animate={{ rotateX: rotate.x, rotateY: rotate.y }} transition={{ type: "spring", stiffness: 150, damping: 20 }} className="relative glass rounded-2xl overflow-hidden transition-colors duration-300" style={{ transformStyle: "preserve-3d", borderColor: isHovered ? `${project.accentColor}20` : undefined }}>
          <div className="absolute top-0 left-0 right-0 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: `linear-gradient(90deg, transparent, ${project.accentColor}, transparent)` }} />
          <div className={`h-44 bg-gradient-to-br ${project.gradient} flex items-center justify-center relative overflow-hidden`}>
            ...
            <motion.div animate={isHovered ? { scale: 1.15, rotate: 5 } : { scale: 1, rotate: 0 }} transition={{ duration: 0.3 }} className="text-5xl opacity-20 group-hover:opacity-40 transition-opacity duration-300" style={{ color: project.accentColor }}>
              <i className={project.icon} />
            </motion.div>
          </div>
          <div className="p-6">
            <h3 className="text-lg font-semibold text-text-white group-hover:text-accent transition-colors duration-200">{project.title}</h3>
            <p className="mt-3 text-sm text-text leading-relaxed">{project.desc}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {project.tech.map((t) => (<span key={t} className="text-[11px] font-mono px-2.5 py-1 rounded-full" style={{ color: project.accentColor, background: `${project.accentColor}10` }}>{t}</span>))}
            </div>
            <div className="mt-5 flex gap-4">
              <a href={project.links.code} className="text-xs text-text hover:text-accent transition-colors flex items-center gap-1.5 font-medium"><i className="fa-brands fa-github" /> Code</a>
              <a href={project.links.live} className="text-xs text-text hover:text-accent transition-colors flex items-center gap-1.5 font-medium"><i className="fa-solid fa-up-right-from-square" /> Live Demo</a>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="container">
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          <motion.div custom={0} variants={fadeUp} className="flex items-center justify-center gap-3 mb-6">
            ...
            <span className="text-[11px] font-mono text-accent tracking-[0.2em] uppercase px-3 py-1 bg-accent-light rounded-full border border-accent/10">03 — Projects</span>
        ...
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.8 }} className="mt-12 text-center">
          <a href="https://github.com/mmmelbahrawy55-beep" target="_blank" className="inline-flex items-center gap-2 px-8 py-3 text-sm font-medium text-text-light glass rounded-full hover:text-text-white hover:border-accent/20 transition-all duration-300 hover:-translate-y-1">
