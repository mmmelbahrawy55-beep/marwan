        ...
        >
          {experiences.map((exp, i) => (
            <motion.div
              ...
              variants={fadeUp}
              whileHover={{ x: 4 }}
              className="group glass rounded-2xl p-6 md:p-8 transition-all duration-300 hover:border-[#5558e6]/15 relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-[#5558e6]/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

              <div className="relative z-10 flex flex-col md:flex-row md:items-start gap-4 md:gap-8">
                <span className="text-4xl font-bold text-[#e2e5f1] group-hover:text-[#5558e6]/20 transition-colors font-mono leading-none">
                  {exp.num}
                </span>

                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 mb-2">
                    <span className="text-xs font-mono text-[#5558e6]">{exp.period}</span>
                    <span className="text-xs text-[#5a6178]/50">•</span>
                    <span className="text-sm font-medium text-[#3d4460]">{exp.company}</span>
                  </div>

                  <h3 className="text-lg md:text-xl font-semibold text-[#1a1d2e] mb-4">
                    {exp.role}
                  </h3>

                  <ul className="space-y-2.5 mb-5">
                    {exp.items.map((item) => (
                      <li key={item} className="text-sm text-[#5a6178] flex items-start gap-3 leading-relaxed">
                        <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#5558e6]/40 flex-shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[11px] font-mono text-[#5558e6]/80 bg-[#5558e6]/[0.05] px-3 py-1 rounded-full"
                      >
                        {tag}
                      </span>
                    ))}
