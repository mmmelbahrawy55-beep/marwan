      ...
      <div className="container">
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          <motion.div custom={0} variants={fadeUp} className="flex items-center justify-center gap-3 mb-6">
            ...
            <span className="text-[11px] font-mono text-accent tracking-[0.2em] uppercase px-3 py-1 bg-accent-light rounded-full border border-accent/10">04 — Experience</span>
            <div className="h-px w-12 bg-gradient-to-l from-transparent to-accent/30" />
          </motion.div>
          <motion.h2 custom={1} variants={fadeUp} className="text-3xl md:text-[2.6rem] font-bold text-text-white">Where I&apos;ve <span className="gradient-text">made an impact</span></motion.h2>
        </motion.div>
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} className="max-w-3xl mx-auto space-y-5">
          {experiences.map((exp, i) => (
            <motion.div key={exp.num} custom={i + 2} variants={fadeUp} whileHover={{ x: 4 }} className="group glass rounded-2xl p-6 md:p-8 transition-all duration-300 hover:border-accent/15 relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-start gap-4 md:gap-8">
                <span className="text-4xl font-bold text-border/50 group-hover:text-accent/20 transition-colors font-mono leading-none">{exp.num}</span>
                <div className="flex-1">
                  ...
                  <h3 className="text-lg md:text-xl font-semibold text-text-white mb-4">{exp.role}</h3>
                  <ul className="space-y-2.5 mb-5">
                    {exp.items.map((item) => (<li key={item} className="text-sm text-text flex items-start gap-3 leading-relaxed"><span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-accent/40 flex-shrink-0" />{item}</li>))}
                  </ul>
                  <div className="flex flex-wrap gap-2">
                    {exp.tags.map((tag) => (<span key={tag} className="text-[11px] font-mono text-accent/80 bg-accent/8 px-3 py-1 rounded-full">{tag}</span>))}
                  </div>
                </div>
              </div>
            </motion.div>
